/*
 * Hand Gesture Controlled Browser Program
 * Code by Ozan Muldur
 * Website: www.ozanmuldur.com
 */
...
{ 
  ...
  duration = pulseIn(echoPin1, HIGH, 5000);// here this pulsein function wont wait more then 5000 micro seconds for the ultrasonic sound to came back. (due to this it wont measure more than 60cm)
                                          // so that, it will return zero if distance greater then 60cm.
 
  r = 3.4 * duration / 2;                  // calculation to get the measurement in cm using the time returned by the pulsein function.  
                                           // the speed of sound in air is about 343 meters per second, roughly 3.4 
                                           // Distance = Speed * Time/2  ==> For the ultrasonic sensor calculation
  distance1 = r / 100.00;                  // Microsecond correction for distance
...
void setup() 
{
  ...
  Serial.println("Hey this is Ozan's Arduino program.");
}
